...
export default function Home() {
  ...
  const { data: messages = [], isLoading: messagesLoading } = trpc.community.messages.useQuery(messageInput, { enabled: isAuthenticated && Boolean(selectedChannelId), refetchInterval: 7000 });
  ...
  return (
    <div className="flex h-screen min-h-[620px] overflow-hidden bg-[#0d0f15] text-slate-200">
      <aside className="flex w-[72px] shrink-0 flex-col items-center gap-3 bg-[#090b10] py-4">
      ...
      <aside className="flex w-[250px] shrink-0 flex-col border-r border-white/5 bg-[#151821]">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-5">
          ...
          <div className="flex items-center gap-2 px-3 py-2 text-sm text-slate-600"><Volume2 size={16} /> Huddle <span className="ml-auto rounded bg-slate-800 px-1.5 py-0.5 text-[9px]">SOON</span></div>
        </div>
        <div className="flex items-center gap-2 border-t border-white/5 bg-[#10121a] px-3 py-3"><div className="relative grid h-8 w-8 place-items-center rounded-full bg-violet-400 text-[10px] font-black text-slate-900">{initials(user.name)}</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-white">{user.name || "Member"}</p><p className="text-[10px] text-emerald-400">● online</p></div><button onClick={() => logout()} title="Log out" className="text-slate-500 hover:text-white"><LogOut size={15} /></button></div>
      </aside>

      <main className="flex min-w-0 flex-1 flex-col bg-[#1a1d27]">
        <header className="flex h-[62px] shrink-0 items-center gap-3 border-b border-white/5 px-6"><Hash size={20} className="text-slate-500" /><h2 className="text-sm font-bold text-white">{selectedChannel?.name ?? "general"}</h2><div className="hidden h-5 w-px bg-white/10 sm:block" /><p className="hidden truncate text-xs text-slate-500 sm:block">A friendly place to start the conversation.</p><div className="ml-auto flex items-center gap-4 text-slate-500"><button title="Invite people" className="hover:text-white"><UserPlus size={18} /></button><button title="Notifications" className="hover:text-white"><Bell size={18} /></button><button title="Pinned messages" className="hover:text-white"><Shield size={17} /></button><button title="Search" className="hidden hover:text-white sm:block"><Search size={18} /></button><button title="Help" className="hidden hover:text-white md:block"><HelpCircle size={18} /></button></div></header>
        <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-7"><div className="mx-auto max-w-4xl">{messagesLoading ? <div className="flex h-full min-h-[300px] items-center justify-center text-sm text-slate-500">Loading messages…</div> : messages.length === 0 ? <div className="flex min-h-[360px] flex-col items-center justify-center text-center"><div className="mb-5 grid h-16 w-16 place-items-center rounded-2xl bg-violet-500/10 text-violet-300"><MessageCircle size={30} /></div><h3 className="text-lg font-bold text-white">Welcome to #{selectedChannel?.name ?? "general"}</h3><p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">This is the beginning of the channel. Send the first message and make it yours.</p></div> : <div className="space-y-6">{messages.map((message, index) => <div key={message.id} className="group flex gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-full text-xs font-black text-slate-900" style={{ backgroundColor: accentAvatars[index % accentAvatars.length] }}>{initials(message.authorName || message.authorEmail)}</div><div className="min-w-0"><div className="flex items-baseline gap-2"><span className="text-sm font-bold text-white">{message.authorName || message.authorEmail || "Member"}</span><span className="text-[10px] text-slate-600">{formatTime(message.createdAt)}</span></div><p className="mt-1 whitespace-pre-wrap break-words text-sm leading-6 text-slate-300">{message.content}</p></div></div>)}<div ref={messagesEndRef} /></div>}</div></div>
        <div className="px-4 pb-5 pt-2 sm:px-7"><div className="mx-auto flex max-w-4xl items-end gap-3 rounded-xl border border-white/5 bg-[#11141c] px-4 py-3 shadow-lg shadow-black/10"><button title="Add attachment" className="mb-1 grid h-7 w-7 place-items-center rounded-full bg-slate-700 text-slate-300 transition hover:bg-violet-500"><Plus size={17} /></button><textarea value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); handleSend(); } }} rows={1} placeholder={`Message #${selectedChannel?.name ?? "general"}`} className="max-h-32 min-h-7 flex-1 resize-none bg-transparent py-1 text-sm text-white outline-none placeholder:text-slate-600" /><button onClick={handleSend} disabled={!draft.trim() || sendMessage.isPending} title="Send message" className="mb-1 text-violet-400 transition hover:text-violet-300 disabled:cursor-not-allowed disabled:opacity-30"><Send size={18} /></button></div><p className="mx-auto mt-2 max-w-4xl text-[10px] text-slate-600">Press Enter to send · Shift + Enter for a new line</p></div>
      </main>

      <aside className="hidden w-[230px] shrink-0 border-l border-white/5 bg-[#151821] px-4 py-5 xl:block"><div className="mb-7 flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-slate-500">Members — 1</p><Settings size={16} className="text-slate-600" /></div><p className="mb-3 px-2 text-[10px] font-bold uppercase tracking-widest text-slate-600">Online — 1</p><div className="flex items-center gap-3 rounded-lg px-2 py-2"><div className="relative grid h-8 w-8 place-items-center rounded-full bg-violet-400 text-[10px] font-black text-slate-900">{initials(user.name)}</div><div><p className="text-xs font-semibold text-slate-300">{user.name || "Member"}</p><p className="text-[10px] text-emerald-400">online</p></div></div><div className="mt-8 rounded-xl border border-violet-500/10 bg-violet-500/[.06] p-4"><WandSparkles size={17} className="text-violet-300" /><p className="mt-3 text-xs font-semibold text-slate-300">Shape the next release</p><p className="mt-1 text-[11px] leading-5 text-slate-500">Voice, invites, roles, and more are queued for the roadmap.</p></div></aside>
    </div>
  );
}
